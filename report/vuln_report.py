...
def build_ref(f: Dict[str, Any], ref_mode: str, ref_path_mode: str, tail_depth: int) -> str:
    ...
    ref_path_mode: full | base | tailN (tail_depth là N)
    """
...
def table(rows,hdrs,w):
    # Tool | Severity | CVE | Title | Ref (full by default)
    # Để Ref không bị cắt: ref_colw=None (không giới hạn)
    tool_w, sev_w, cve_w, title_w = 12, 8, 16, 68
    ref_colw = None  # None => không cắt Ref
    # Nếu muốn khống chế bề rộng tổng: giảm title_w theo w, còn Ref vẫn full
    ...
    print(DIM + " ".join([
        "─"*tool_w, "─"*sev_w, "─"*cve_w, "─"*title_w, "─"*8  # gợi ý, Ref không cắt nên vạch 8
    ]) + RESET)
    for r in rows:
        ref = r[4] if ref_colw is None else colfit(r[4], ref_colw)
        print(" ".join([
...
def main(argv: List[str]) -> int:
    ...
    ref_mode="auto"; ref_path_mode="tail2"; ref_tail_depth=2
    ...
    for path in files:
        ...
        try:
            ...
            for f in PARSERS[fmt](obj, source=p.name):
                ...
                if skip_empty and (not f.get("title") or f["title"].strip().lower() in ("na","unknown","gitleaks","semgrep finding","trivy vulnerability","dependency-check finding")):
                    continue
    ...
    if findings:
        ...
        for f in findings_sorted:
            ref_val = build_ref(f, ref_mode, ref_path_mode if ref_path_mode!="tail" else f"tail{ref_tail_depth}", ref_tail_depth)
            rows.append([
                f.get("tool",""),
