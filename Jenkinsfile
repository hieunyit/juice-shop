      ...
      steps {
        withAWS(credentials: 'aws-jenkins', region: 'ap-southeast-1') {
          sh ''' 
            ...
            aws ecs update-service --cluster juice-shop-cluster --service juice-shop-svc --task-definition juice-shop-task --output json --capacity-provider-strategy capacityProvider=FARGATE_SPOT,weight=1,base=0 --force-new-deployment
          '''
        }
      }
    }
